\documentclass[twocolumn, a4paper, 10pt]{article} % Thêm twocolumn và giảm size chữ xuống 10pt cho chuẩn báo khoa học
...
\usepackage{float}
\usepackage{hyperref}
\usepackage{titlesec} % Để chỉnh sửa khoảng cách tiêu đề nếu cần

% Cấu hình lề trang (Giảm lề một chút để cột rộng hơn)
\geometry{
 a4paper,
 left=15mm,
 right=15mm,
...
The dataset used for this practical work is the \textbf{MIT-BIH Arrhythmia Database} (downloaded from Kaggle).
\begin{itemize}
...
Instead of using Deep Learning (CNN), I experimented with a classic Machine Learning approach: the \textbf{Random Forest Classifier}. 
...
The model was implemented using Python and the \texttt{scikit-learn} library.
\begin{itemize}
...
The model was evaluated on the Test set (21,892 samples) and achieved excellent results:
\begin{itemize}
...
Below is the detailed performance for each class based on the experiment results:

% Dùng resizebox để bảng tự co nhỏ cho vừa cột
\begin{table}[ht]
\centering
...
Figure \ref{fig:matrix} visualizes the performance of the model across all classes. It helps identify where the model makes errors (e.g., distinguishing between Fusion beats and Normal beats).

\begin{figure}[ht]
    \centering
    % Dùng columnwidth thay vì textwidth
...
According to the dataset description, the original paper using a Convolutional Neural Network (CNN) achieved an accuracy of approximately \textbf{93.4\%}.

My implementation using Random Forest achieved \textbf{97.30\%}, which is higher than the baseline. This suggests that for this specific pre-processed version of the MIT-BIH dataset, ensemble methods like Random Forest can perform exceptionally well without the high computational cost of Deep Learning.

\section{Conclusion}
In this practical work, I successfully built a Random Forest model to classify arrhythmia heartbeats. The model reached an accuracy of 97.30\%, surpassing the reference baseline. While the model performs perfectly on Normal beats (Recall = 1.00), it still faces challenges with minority classes like Fusion beats (Recall = 0.58), which could be improved in future work by using more advanced data augmentation techniques.
